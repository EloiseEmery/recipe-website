...
function RecipeCard({ recipe, isIngredientsMode = false, searchState = null }) {
  const hasCookTime =
  ...
  const hasIngredientMatchData =
    isIngredientsMode || hasMissedIngredientCount;
  ...
  return (
    <Link
      to={`/recipe/${recipe.id}`}
      state={searchState ? { fromSearch: searchState } : undefined}
      className="group flex h-full flex-col overflow-hidden rounded-3xl border border-stone-200/80 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200"
    >
      ...
      <div className="flex flex-1 flex-col space-y-3 p-4 sm:p-5">
        ...
        {missingIngredientPreview.length > 0 && (
          <p className="mt-auto text-sm text-stone-600">
